...
class Dataset:
    def __init__(self, run_label, gases, data_path='/gpfs/gibbs/project/david_moore/aj487/Data_WL110/Outgassing_Setup/20231106/'):
    ...
    '''def unit_step(self,t):
        if t < 43000:
            return 0
        elif t >= 43000 and t <= 85000:
            return 1
        elif t >= 87500 and t <= 127500:
            return 2
        elif t >= 131000 and t <= 175000:
            return 3'''
    
    def unit_step(self, t):
        result = np.zeros_like(t)  # Initialize an array of zeros with the same shape as t
        result[t >= 43000] = 1
        result[t >= 87500] = 2
        result[t >= 131000] = 3
        return result
        
    def fitfunction_short_thermal_conductivity(self,tT,a,b,c,e):
        return a*(np.exp((- b)/(297+10*self.unit_step(tT[0][0])*(1 - np.exp(-e*tT[1]))))/(tT[0]+c))**(0.5)
